  ...
  const handleAddQuote = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/quotes', {
        ...
        body: JSON.stringify({ text: newQuoteText, source: newQuoteSource, active: true, tags: [] })
      });
      if (res.ok) {
        const added = await res.json();
        setDbQuotes([added, ...dbQuotes]);
        setNewQuoteText("");
        setNewQuoteSource("");
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-10">
            <div className="p-8">
              ...
              <blockquote className="text-xl italic text-center text-gray-700 mb-6 relative">
                ...
                <p className="px-10 py-2">{allQuotes[currentQuote]?.text}</p>
              ...
              {showAddForm && (
                <form onSubmit={handleAddQuote} className="mt-8 bg-purple-50 p-6 rounded-lg text-left">
                  <h3 className="font-semibold text-purple-800 mb-4">Add a Divine Quote</h3>
                  <div className="mb-4">
                    <label className="block text-sm text-gray-700 mb-1">Quote Text</label>
                    <textarea 
                      required
                      value={newQuoteText}
                      onChange={e => setNewQuoteText(e.target.value)}
                      className="w-full border p-2 rounded focus:ring-purple-400 focus:border-purple-400"
                    />
                  </div>
                  <div className="mb-4">
                    ...
                    <input 
                      ...
                      className="w-full border p-2 rounded focus:ring-purple-400 focus:border-purple-400"
                      placeholder="e.g. Srimad-Bhagavatam 1.1.1"
                    />
                  </div>
                  <button type="submit" className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700">Submit Quote</button>
                </form>
              )}
            </div>
          </div>
